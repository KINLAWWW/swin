...
class SoftLabelDataset(Dataset):
    def __init__(self, original_dataset,num_classes):
        ...
        self.label = torch.tensor([ y for _,y in original_dataset])

        self.targets = F.one_hot(self.label).float().requires_grad_(False)

        self.logits = torch.zeros(len(original_dataset),num_classes).requires_grad_(False)
        

    def reset(self):
        self.targets = F.one_hot(self.label).requires_grad_(False)

    def __setitem__(self,index, pred):
        self.logits[index.cpu()] = pred.cpu().detach()
    
    def update_target(self,T):
        'T: num of SEAL steps completed at the end of epoch'
        if T <=0:
            return 
        self.targets = self.targets *(T/(T+1)) + self.logits/(T+1)

    def __len__(self):
        return len(self.original_dataset)
    
    def __getitem__(self, index) -> Any:
        return self.original_dataset[index][0], self.targets[index], self.label[index],index
...
class SEALClassifierTrainer(pl.LightningModule):
    r'''
    ...
    def prepare_train_dataloader(self,train_dataset,batch_size,shuffle = False): 
        soft_label_dataset = SoftLabelDataset(original_dataset= train_dataset,num_classes=self.num_classes)
        self.train_dataset = soft_label_dataset
        return DataLoader(soft_label_dataset,batch_size =batch_size,shuffle = shuffle)
    ...
    def fit(self,
        ...
        '''
        self.seal_update_epochs = seal_update_epochs
    ...
    def training_step(self, batch: Tuple[torch.Tensor],
        ...
        x, y,label,index = batch
        y_hat = self(x)
        loss = self.ce_fn(y_hat, y)

        if self.current_epoch% self.seal_update_epochs == 0:
            self.train_dataset[index] = y_hat
    ...
    def on_train_epoch_end(self) -> None:
        if self.current_epoch% self.seal_update_epochs == 0:
            self.train_dataset.update_target(self.current_epoch)
