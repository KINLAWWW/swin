...
class DLClassifierTrainer(pl.LightningModule):
    r'''
    ...
    def training_step(self, batch: Tuple[torch.Tensor],
        ...
        y_hat = torch.empty(0,device=x.device)
        if x1.any():
            y1_hat = self.model1(x1)
            y_hat = y1_hat
                
        if x2.any():
            y2_hat = self.model2(x2)
            y_hat = torch.cat([y_hat,y2_hat],dim=0)
        
        loss = self.ce_fn(y_hat, y_task)
    ...
    def validation_step(self, batch: Tuple[torch.Tensor],
        ...
        self.val_metrics.update(y_hat, y_task)
        return loss
    ...
    def test_step(self, batch: Tuple[torch.Tensor],
        ...
        self.test_metrics.update(y_hat, y_task)
        return loss
    ...
    def configure_optimizers(self):
        parameters = list(self.model1.parameters())+ list(self.model2.parameters())
