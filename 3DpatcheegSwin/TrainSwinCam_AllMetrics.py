...
cv = KFoldPerSubjectGroupbyTrial(n_splits=4, shuffle=True,split_path='.torcheeg/model_selection',random_state=42)

metrics = ['accuracy', 'recall', 'precision', 'f1score','kappa']
csv_path = 'logs/SwinCam_testmetrics_results.csv'
...
# 保存新结果
grouped.to_csv('logs/SwinCam_testmetrics_results_AVG.csv', index=False, float_format='%.4f')
print("✅ 每 4 行平均结果已保存到 logs/SwinCam_testmetrics_results_AVG.csv")
