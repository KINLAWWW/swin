...
from loss_functions import compute_attention_map, get_prefrontal_mask, combined_loss
...
class ClassifierTrainer(pl.LightningModule):
    ...
    def fit(self,
        ...
        trainer = pl.Trainer(devices=self.devices,
                             ...
                             enable_checkpointing=False, # change
                             logger=False, # change
                             max_epochs=max_epochs,  # 添加这一行
                             *args,
                             **kwargs)
    ...
    def training_step(self, batch: Tuple[torch.Tensor],
        ...
        activation_maps = compute_attention_map(
            self.model, x)
        
        # Compute combined loss (region-wise + sample-wise)
        loss = combined_loss(
            outputs=y_hat,
            labels=y,
